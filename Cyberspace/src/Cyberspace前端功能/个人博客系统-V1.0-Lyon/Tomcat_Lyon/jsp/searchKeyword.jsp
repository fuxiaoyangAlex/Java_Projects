...
$(addThank = function(){
		...
		$(".blog-post").on("click","#submitThank",function(){
				...
				var thanknode = thisnode.children("span");
				var originNum = parseInt(thanknode.text());
				var articleId = thanknode.parents(".linkandshare-catarticle").children(".articleId").text();
				var articleTitle = thanknode.parents(".linkandshare-catarticle").children(".articleTitle").text();
				$.get(
						"forecheckLogin",
						function(result){
								if("success"==result){
									...
									xmlhttp.send("articleId="+articleId+"&articleTitle="+articleTitle);
								}else{
										alert("亲，请先登录后再点❤️哦！");
								}
						});
	    ...
	    <div class="col-sm-8 blog-main">
	        <!--文章主体全部在blog-post中 ，要添加新文章，直接copy此div模块即可-->
					<c:forEach items="${articles}" var="article" varStatus="st">
			 				<div class="blog-post">
			 						 <a href="/Lyon/foreviewSingleBlog?Id=${article.getId()}"target="_blank"><h2 class="blog-post-title"><strong style="font-family:Andale Mono;color:#7171C6"><c:out value="${article.getTitle()}"/></strong></h2></a>
									 <div>
                     <c:forEach items="${article.getarticleTags()}" var="tag">
									 ...
									 <div class="linkandshare-catarticle">
		 								  ...
		 								  <c:if test="${article.getthankNum()==0}">
		 									    <a href="javascript:void(0)" id="submitThank" onclick="addThank()" style="margin-right:50px;color:#919191;"><span class="glyphicon glyphicon-heart" ><span>${article.getthankNum()}</span></span>感谢</a>
		 							    </c:if>
		 								  <c:if test="${article.getthankNum()!=0}">
		 									    <a href="javascript:void(0)" id="submitThank" onclick="addThank()" style="margin-right:50px;"><span class="glyphicon glyphicon-heart" style="color:#EE2C2C;"><span>${article.getthankNum()}</span></span>感谢</a>
		 							    </c:if>
		 								  <c:if test="${article.getcommentNum()==0}">
		 			                <a href="" style="margin-right:50px;color:#919191;"><span style=""><img src="images/mother11.png" height="16" width="16">${article.getcommentNum()}</span>评论</a>
		 							    </c:if>
		 								  <c:if test="${article.getcommentNum()!=0}">
